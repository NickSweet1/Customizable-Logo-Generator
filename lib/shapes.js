...
class Shape {
    ...
    getShapeEl() {
        ...
        switch (this.shapeType) {
            case "Circle":
                shapeEl = `<circle cx="${centerX}" cy="${centerY}" r="50" stroke="${this.borderColor}" stroke-width="5" fill="${this.shapeColor.replace(/\s/g, "")}"/>`;
                break;
            case "Square":
                shapeEl = `<rect x="100" y="25" width="100" height="100" stroke="${this.borderColor}" stroke-width="5" fill="${this.shapeColor.replace(/\s/g, "")}"/>`;
                break;
            case "Triangle":
                shapeEl = `<polygon points="150,15 215,100 85,100" stroke="${this.borderColor}" stroke-width="5" fill="${this.shapeColor.replace(/\s/g, "")}"/>`;
                break;
        }
        return shapeEl;
    }
