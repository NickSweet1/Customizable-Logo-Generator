...
generateText = (abbr, textColor) => {
    return `<text x="50%" y="50%" font-family="Verdana" font-size="35" fill="${textColor}" text-anchor="middle" dominant-baseline="middle">${abbr}</text>`;
}
...
class Shape {
    ...
    getShapeEl() {
        ...
        switch (this.shapeType) {
            case "Circle":
                shapeEl = `<circle cx="${centerX}" cy="${centerY}" r="50" fill="${this.shapeColor}"/>`;
                break;
            case "Square":
                shapeEl = `<rect x="100" y="25" width="100" height="100" fill="${this.shapeColor}"/>`;
                break;
            case "Triangle":
                shapeEl = `<polygon points="150,15 215,100 85,100" fill="${this.shapeColor}"/>`;
                break;
        }
        return shapeEl;
    }
